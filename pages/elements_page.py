...
class TextBoxPage(BasePage):
    locators = TextBoxPageLocators()  # общая переменная для локаторов

    @allure.title("fill all field")
    def fill_all_field(self):
        info = next(get_person())  # функцию берём из генератора 11 строка
        full_name = info.full_name
        email = info.email
        current_address = info.current_address
        permanent_address = info.permanent_address
        with allure.step('Filled all'):
    ...
    def check_filled_field(self):  # проверяет заполненность полей
...
class CheckBoxPage(BasePage):
    ...
    def get_checked_box(self):  # тут не прошло
...
class WebTablePage(BasePage):
    ...
    def add_new_person(self, count=1):  # кол-во человек один
        while count != 0:
            info = next(get_person())  # next аналогично range
    ...
    @allure.title("check new added person")  # проверяем ,что добавился
    ...
    @allure.title("search people")  # ищем через лупу на сайте
    def search_people(self, key_word):
        self.element_is_visible(self.locators.SEARCH_INPUT).send_keys(key_word)

    @allure.title("check found person")  # находим,а потом удаляем
    ...
    def update_person_info(self):
        ...
        self.element_is_visible(self.locators.AGE_INPUT).clear()  # чистим значение
    ...
    @allure.title("check deleted person")  # проверяем ,что удалили,пусто
...
class LinksPage(BasePage):
    ...
    def click_on_simple_link(self):
        ...
        link_href = simple_link.get_attribute('href')  #получаем линку
        response = requests.get(link_href)
        if response.status_code == 200:              #можно и не проверять статус код,немного  API затронули
            simple_link.click()
            self.driver.switch_to.window(self.driver.window_handles[1]) #используем JS. метод переходит по вкладкам,использует список,[0]линка с которой переходим,[1]куда
            url = self.driver.current_url
            return link_href, url
        else:
            return response.status_code

    def click_on_simple_link_v2(self):                #без проверки статус-кода
        simple_link = self.element_is_visible(self.locators.SIMPLE_LINK)
    ...
    def click_on_the_not_found(self, url):
        response = requests.get(url)
        if response.status_code == 200:
            self.element_is_present(self.locators.BAD_REQUEST_LINK).click()    #не тот локатор тут
        else:
            return response.status_code
...
class DownloadPage(BasePage):
    ...
    def download_file(self):
        link = self.element_is_present(self.locators.DOWNLOAD_FILE).get_attribute('href')  #скачиваем и имитируем атрибуты
        link_b = base64.b64decode(link) #декодируем по этой линке
        path_name_file = rf"C:\Users\aorlova\PycharmProjects\my_autotest2\my_autotest2\test{random.randint(0, 999)}.jpg" #путь,куда закидыать скаченные файлы
        with open(path_name_file, 'wb+') as f: #бинарный путь
            offset = link_b.find(b'\xff\xd8')
            f.write(link_b[offset:])   #записываем
            check_file = os.path.exists(path_name_file)
            f.close()  #можно не закрывать
        # os.remove(path_name_file) #удаляем наш путь и файл сразу,можно закомитить,чтобы видеть скаченные файлы
        return check_file
...
class DynamicPage(BasePage):
    ...
    def check_enable_button(self):
        try:
            self.element_is_clicable(self.locators.ENABLE_AFTER_FIVE_SECOND, 5)  #проверяем на кликабельность,таймаут 5сек,ждём пока станет кликабельной
        except TimeoutException:  #иначе отдаём ошибку
            return "Timeout"
        return True

    @allure.step("check changed of color")
    def check_changed_of_color(self):                   #ждём пока кнопка не поменяет цвет
        color_button = self.element_is_present(self.locators.COLOR_CHANGE_BUTTON) #получаем кнопочку
        color_button_before = color_button.value_of_css_property("color")  #сюда придёт свойство кнопки color в начале
        time.sleep(6)
        color_button_after = color_button.value_of_css_property("color") #получаем цвет кнопки потом
