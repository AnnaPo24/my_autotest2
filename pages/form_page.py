...
class FormPage(BasePage):
    ...
    def fill_date_form(self):
        calender = self.element_is_visible(self.locators.BIRTH_DAY)  #полностью нашли календарь
        calender.click()
        with allure.step("Select year"):
            self.element_is_visible(self.locators.YEARS_CHANGE).click()  #нахожу по году,нажимаю на него
            year_change = self.element_is_visible(self.locators.YEARS)
            year = year_change.text     #сохраняю значение
            year_change.click()  #и кликаю по нему
    ...
    def fill_other_field(self):
        ...
        for item in subject_list:
            self.element_is_visible(self.locators.SUBJECT).send_keys(item) #вписываю в поле
            self.element_is_visible(self.locators.SUBJECT).send_keys(Keys.RETURN)    #нажимаю энтер keys.RETURN
    ...
    def form_result(self):
        result_list = self.elements_are_visible(self.locators.ALL_TABLE)   #используем строку
        data = ""
